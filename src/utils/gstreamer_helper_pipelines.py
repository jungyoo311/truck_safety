'''
GStreamer pipeline building utilities providing modular components for video processing applications.
Creates reusable pipeline strings for source input, AI inference, object tracking, and display output with customizable parameters.
Supports multiple input sources (USB, RPi camera, files) and processing stages for computer vision applications.
'''
import os
...
def OVERLAY_PIPELINE(name='hailo_overlay'):
    ...
    #     f'textoverlay name={name}_textoverlay font-desc="Sans, 24" '
    #     f'valignment=top halignment=left text="TESTING OVERLAY: Speed Display" !'
    # )
    # def OVERLAY_PIPELINE(name='hailo_overlay'):
    """
    Creates a GStreamer pipeline string for the hailooverlay element.
    ...
    overlay_pipeline = (
        f'{QUEUE(name=f"{name}_q")} ! '
        # f'hailooverlay name={name} qos=false display-labels=true show-boxes=true box-color=red text-color=white font-size=12 min-confidence=0.6! '
        f'hailooverlay name={name} '
    )
    print(f"✅ [DEBUG] Modified Overlay Pipeline: {overlay_pipeline}")  # Debugging Output
